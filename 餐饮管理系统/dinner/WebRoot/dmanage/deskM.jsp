...
<html>
  ...
  <body>
    ...
    <div id="content" >
    	<div class="context">
    	<div class="left"><a href="DeskServlet?action=add" class="a_demo">添加餐桌</a> </div>
			<div class="center">
				<strong>
	   			<c:choose>
				...
				<a href="javascript:showadd()" class="a_demo">统计查询</a> &nbsp;&nbsp;&nbsp;
    ...
    <thead>
	  ...
	  <td>${desk.did }</td>
	  <td>${desk.dname }</td>
	  <td>${desk.dstatus eq 0?'空桌':desk.dstatus}
	      ${desk.dstatus==0?'':'号订单使用中' }
	  </td>
	  <td>${desk.dexist eq 1?'在用':'停用' }</td>
	  <td>
	  <!-- 调用deskManageServlet 修改餐桌 -->
	  <a href="DeskServlet?action=update&did=${desk.did}" class="a_demo">修改餐桌名</a> | 
	  <c:choose>
	  <c:when test="${desk.dexist eq 1}">
	      <a href="javascript:banD(${desk.did },${desk.dstatus },${page.pageNum } )" class="a_demo">停用该餐桌</a>
	  </c:when>
	  <c:otherwise>
	       <a href="javascript:useD(${desk.did },${page.pageNum })" class="a_demo">启用该餐桌</a></c:otherwise>
	  </c:choose>
	  </td>
	  </tr>
	  </c:forEach>
	  ...
	  <table width="950" height="50" border="0" cellpadding="0" cellspacing="0" style="border-top:1px #C3C3C3 solid; position:fixed;bottom:0px; width: 880;">
  		...
  		<td align="center" style="text-align:center;">
	  		<a href="DeskServlet?pageNum=${page.first }">首页</a>&nbsp;&nbsp;&nbsp;
    		<a href="DeskServlet?pageNum=${page.up }">上一页</a>&nbsp;&nbsp;&nbsp;
    		当前第${page.pageNum}页，共计${page.count }页&nbsp;&nbsp;&nbsp;
    		<a href="DeskServlet?pageNum=${page.down }">下一页</a>&nbsp;&nbsp;&nbsp;
    		<a href="DeskServlet?pageNum=${page.last }">末页</a>&nbsp;&nbsp;&nbsp;
	  	</td>
	  	</tr>
	  </table>
	  </div>
